...
/**
 * MODIFIED FOR USAGE WITH BEDROCK AND LAGOON BY ICON AGENCY
 * 
...
if ( stripos( $_SERVER['REQUEST_URI'], '/app/uploads/' ) !== false && getenv('LAGOON_ENVIRONMENT_TYPE') !== 'production') sfp_expect();
...
function sfp_get_relative_path() {
	static $path;
	if ( !$path ) {
		$path = preg_replace( '/.*\/app\/uploads(\/sites\/\d+)?\//i', '', $_SERVER['REQUEST_URI'] );
	}
	/**
